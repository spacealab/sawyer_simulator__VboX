...
class PickAndPlace(object):
    def __init__(self, limb="right", hover_distance=0.15, tip_name="right_gripper_tip"):
        ...
        # cache joint names for convenience
...
class PoseSubscriber(object):
    def __init__(self):
        self.pose = None
        self.subscriber = rospy.Subscriber("/box_pose", Pose, self.callback, queue_size=1)
        rospy.loginfo("Subscribing to /box_pose topic...")

    def callback(self, message):
        if self.pose is None:  # Only store the first received pose
            self.pose = message
            rospy.loginfo(f"Received pose: {self.pose.position.x}, {self.pose.position.y}, {self.pose.position.z}")
            # Unregister subscriber after receiving the first message to stop listening
            self.subscriber.unregister()
            rospy.loginfo("Unregistered from /box_pose topic.")
...
def main():
    ...
    This version waits for a dynamic pose from the /box_pose topic.
    """
    rospy.init_node("ik_pick_and_place_dynamic_demo")

    # Initialize the pose subscriber
    pose_sub = PoseSubscriber()

    # Wait until a pose is received
    print("Waiting for a pose to be published on /box_pose topic...")
    while pose_sub.pose is None and not rospy.is_shutdown():
        rospy.sleep(0.1)
    ...
    # The Pose of the block, received from the topic
    # The orientation is kept fixed for simplicity
    block_pose = pose_sub.pose
    block_pose.orientation = Quaternion(
                             x=-0.00142460053167,
                             y=0.999994209902,
                             z=-0.00177030764765,
                             w=0.00253311793936)
    ...
    # Perform a single pick and place
    print(f"Picking object at (x={block_pose.position.x}, y={block_pose.position.y}, z={block_pose.position.z})")
    pnp.pick(block_pose)

    # Define a placing pose (e.g., move it to another location)
    # For this demo, we'll place it at a fixed offset from the pick location
    place_pose = copy.deepcopy(block_pose)
