...
def main():
    ...
    # Argument Parser
    parser = argparse.ArgumentParser(description='Pick and place demo with static coordinates')
    parser.add_argument('--x', type=float, default=0.45, help='X coordinate of the block')
    parser.add_argument('--y', type=float, default=0.155, help='Y coordinate of the block')
    parser.add_argument('--z', type=float, default=-0.129, help='Z coordinate of the block')
    args = parser.parse_args(rospy.myargv()[1:])

    rospy.init_node("ik_pick_and_place_static_demo")
    ...
    # The Pose of the block, read from command-line arguments
    block_pose = Pose(
    ...
    print(f"Picking object at (x={args.x}, y={args.y}, z={args.z})")
    pnp.pick(block_pose)
    ...
    place_pose = Pose(
        position=Point(x=args.x + 0.15, y=args.y - 0.1, z=args.z),
        orientation=overhead_orientation)
